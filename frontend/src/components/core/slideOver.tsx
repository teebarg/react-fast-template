...
const SlideOver: React.FC<SlideoverProps> = ({ isOpen, onClose, children, title, className, footer, size = "w-1/4", direction = "right" }) => {
        ...
        <OverlayContainer>
            ...
            <FocusScope contain restoreFocus autoFocus>
                <div
                    ...
                    {...modalProps}
                    ref={ref}
                    className={`fixed top-0 ${size} h-screen shadow-lg transform transition-transform duration-300 z-50 ${className} ${location[direction]}`}
                >
                    <Button isIconOnly onPress={onClose} className="absolute top-4 right-4 bg-transparent">
                        <CancelIcon size={24} />
                    </Button>
                    <div className="flex flex-col h-full">
                        {title && <div className="text-2xl mb-4 font-semibold pl-2">{title}</div>}
                        <ScrollShadow className="w-full h-full max-h-full flex-1">
