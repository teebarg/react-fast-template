...
[NexUI](https://next-ui.org/). `NexUI` is a fully-loaded component library, super customizable, and easy to use.
...
The [theme system](./) is based on [NextUI Theme](https://next-ui). To get the current theme mode or to change it you can use `useTheme` hook:
